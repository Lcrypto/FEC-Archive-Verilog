/*
  ...
  llr_t                            ldpc_dec_vnode__iuLLR       [pLLR_BY_CYCLE] ;
...
// Workfile      : ldpc_dec_sbuf_vnode.v
// Description   : LDPC decoder variable node arithmetic top module: read cnode and count vnode. Consist of pLLR_BY_CYCLE engines.
//                 This module has interface for for direct uploading of initial LLR to shift ram
//

`include "define.vh"

module ldpc_dec_sbuf_vnode
(
  iclk    ,
  ireset  ,
  iclkena ,
  ...
  input  logic                        iusop                       ;
  input  logic                        iuval                       ;
  input  llr_t                        iuLLR       [pLLR_BY_CYCLE] ;
  //
  output logic                        oval                        ;
  ...
  always_ff @(posedge iclk or posedge ireset) begin
    if (ireset)
      ...
      oval <= iuval | engine__oval[0];
  end

  always_ff @(posedge iclk) begin
    if (iclkena) begin
      if (iuval | engine__oval[0]) begin
        for (int c = 0; c < pC; c++) begin
//        omask[c]  <= Hb[c][engine__otcnt[0]][31]; // not need, this mask moved to cnode input
          for (int llra = 0; llra < pLLR_BY_CYCLE; llra++) begin
            ovnode[c][llra] <= iuval ? (iuLLR[llra] <<< (cNODE_W - pLLR_W)) : engine__ovnode[llra][c];
            waddr           <= (iusop | engine__osop[0]) ? '0 : (waddr + 1'b1);
          end
        end
      end
    end
