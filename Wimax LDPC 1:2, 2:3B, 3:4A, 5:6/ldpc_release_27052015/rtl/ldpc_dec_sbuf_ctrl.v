...
// Workfile      : ldpc_dec_sbuf_ctrl.v
// Description   : Main FSM for decoder with splited input buffer.
//                Generate address generator & vnode/cnode engines control signals
//

`include "define.vh"

module ldpc_dec_sbuf_ctrl
(
  iclk         ,
  ireset       ,
  iclkena      ,
  ...
  always_ff @(posedge iclk or posedge ireset) begin
    if (ireset) begin
      ...
      case (state)
        ...
        //
        cWAIT_STATE       : state <= ibuf_full  ? cHSTEP_STATE      : cWAIT_STATE;
        //
        cHSTEP_STATE      : state <= step.done  ? cWAIT_HDONE_STATE : cHSTEP_STATE;
        cWAIT_HDONE_STATE : state <= tcnt.done  ? cVSTEP_STATE      : cWAIT_HDONE_STATE;
        //
        cVSTEP_STATE      : state <= step.done  ? cWAIT_VDONE_STATE : cVSTEP_STATE;
        cWAIT_VDONE_STATE : state <= iter.last  ? cWAIT_STATE       : cHSTEP_STATE;
        //
        cRSVD0_STATE      : state <= cWAIT_STATE;
        cRSVD1_STATE      : state <= cWAIT_STATE;
      endcase
    end
  end
  ...
  always_ff @(posedge iclk or posedge ireset) begin
    if (ireset) begin
      ...
      cnode_busy  <= icnode_busy;
      obuf_rempty <= (state == cVSTEP_STATE) & iter.last & cnode_busy & !icnode_busy; // negedge of icnode_busy
    end
  end
  ...
  always_ff @(posedge iclk) begin
    if (iclkena) begin
      ...
      else if (state == cHSTEP_STATE & step.done) begin
