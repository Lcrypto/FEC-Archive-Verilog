...
(
  ...
  logic signed   [pLLR_W-1 : 0] buf_oLLR [pLLR_BY_CYCLE] /* synthesis keep */;
...
//assign mem__irtag[5 : 3]            = {ctrl__ocnode_eop, ctrl__ocnode_val, ctrl__ocnode_sop};
  ...
  assign mem__iwrite  = vnode__oval | cnode__oval ;
  ...
  always_ff @(posedge iclk) begin
    if (iclkena) begin
      ...
      // set mode
      if (ctrl__ovnode_val & ctrl__ovnode_sop)
        vnode__iload <= ctrl__oload_mode;
    end
  end
