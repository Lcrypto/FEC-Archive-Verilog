...
// Workfile      : gsfc_ldpc_dec_addr_gen.v
...
`endif
    ...
    maddr_tab = get_maddr_tab(addr_tab);
  end
        ...
        if (ic_nv_mode) begin
          ...
          for (int c = 0; c < pC; c++) begin
            ...
            for (int w = 0; w < pW; w++) begin
              for (int n = 0; n < pNODE_BY_CYCLE; n++) begin
                raddr[c][w][n].baddr <= maddr_tab[c][w][n][tcnt2tab[c][n]][0].baddr + zcnt.val;
                for (int llra = 0; llra < pLLR_BY_CYCLE; llra++) begin
                  raddr[c][w][n].sela   [llra] <= maddr_tab[c][w][n][tcnt2tab[c][n]][0].sela     [llra]; // addr gen use for read address -> use sela
                  raddr[c][w][n].offset [llra] <= maddr_tab[c][w][n][tcnt2tab[c][n]][0].offsetm  [llra]; // it's mask, for overlap logic  (!!!)
                end
              end
            end
          end
