...
// Date          : $Date$
// Workfile      : bch_berlekamp_ibm_2t_by_t.v
...
`ifdef __USE_ALTERA_MACRO__
  state_t state, state_next /* synthesis syn_encoding = "sequential"*/;
`else
...
`endif
  ...
  always_comb begin
    ...
    case (state)
      ...
      cSTEP2_STATE  : state_next = t_cnt_done     ? cSTEP3_STATE  : cSTEP2_STATE; // long delay state
      cSTEP3_STATE  : state_next = cSTEP4_STATE ;
      cSTEP4_STATE  : state_next = t_cnt_done     ? (cnt_done ? cWAIT_STATE   : cSTEP1_STATE)  : cSTEP4_STATE; // long delay state
`ifdef __USE_ALTERA_MACRO__
      cSTEP5_STATE  : state_next = cWAIT_STATE;
      cSTEP6_STATE  : state_next = cWAIT_STATE;
`endif
    endcase
  end
  ...
  always_ff @(posedge iclk) begin
    if (iclkena) begin
      ...
      if (state == cWAIT_STATE & isyndrome_val)
...
//          b_poly[1 : t] <= dat2mult_12_b[0 : t-1]; but there is one more shift that is why use [1 : t]
  ...
  function data_t deg (input vector_t vector);
    deg = 0;
    for (int i = 0; i <= t; i++) begin
      if (vector[i] != 0)
        deg = i;
