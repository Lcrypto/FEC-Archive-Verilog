...
// Date          : $Date$
// Workfile      : bch_berlekamp_ribm_t_by_t.v
...
(
  ...
  always_comb begin
    ...
    case (state)
      ...
      cSTEP1_STATE : state_next = (cnt_done & tcnt_done)  ? cWAIT_STATE   : cSTEP1_STATE;
    endcase
  end
    ...
    if (iclkena) begin
      ...
      if (state == cWAIT_STATE) begin
        ...
        for (int i = 0; i <= t2; i++) begin
          if (tcnt_done)  // round shift one position + shift left 2 position
            tetta_clear[i] <= (i < t2-2) ? tetta_clear[i+3] : ((i == t2) ? tetta_clear[0] : '0);
          else            // round shift one position
