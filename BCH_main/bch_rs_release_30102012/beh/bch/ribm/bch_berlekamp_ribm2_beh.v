...
(
  ...
  initial begin : main
    ...
    forever begin
      ...
      delta[t2 : t3+1] = '{t3 : 1, default : 0};

      tetta[0  : t2-1] = syndrome[0 : t2-1];
      tetta[t2 : t3]   = '{t3 : 1, default : 0};
      //
      ...
      for (int r = 0; r <= t2-1; r += 1) begin
        ...
        for (int i = 0; i <= t3; i++) begin
          ...
          $display("mult(%0d, %0d) ^ mult(%0d, %0d) = %0d", sigma, delta[i+1], delta[0], tetta[i], delta_next[i]);
        end
        // step 2
        if ((delta[0] != 0) && (kv >= 0)) begin
          tetta_next [0 : t3] = delta [1 : t3+1]; // mult by z
          sigma               = delta[0];
          kv                  = -kv - 1;
        end
        else begin
