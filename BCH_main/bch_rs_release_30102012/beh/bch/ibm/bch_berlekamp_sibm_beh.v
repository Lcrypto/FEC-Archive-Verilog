...
(
  ...
  initial begin : main
    ...
    forever begin
      ...
      //
      syndrome[1    : t2] = isyndrome[1 : t2];
      syndrome[-t+1 :  0] = '{default : 0};
      //
      ...
      for (int r = 0; r < t; r += 1) begin
        ...
        for (int j = 0; j <= t; j++) begin
          delta ^= gf_mult_a_by_b(l_poly[j], syndrome[2*r+1-j]);
        end
        // step 2
        for (int i = 0; i <= t; i++) begin
        ...
        if ((delta != 0) && (kv >= 0)) begin
          b_poly_next[0 : t]  = l_poly[-1 : t-1]; // mult by z
          sigma               = delta;
          kv                  = -kv;
        end
        else begin
