...
module rs_eras_berlekamp_ribm_beh
(
  iclk          ,
  iclkena       ,
  ireset        ,
...
//end
  ...
  initial begin : main
    ...
    forever begin
      ...
      if (pLOG_ON) $display(str);
      //
      $sformat(str, "ribma get %0d erasures : ", eras_num);
      ...
      vector_logout("ribma init b poly ",  b_poly[0 : t2]);
      //------------------------------------------------------------------------------------------------------
      ...
      for (int r = 1; r <= eras_num; r++) begin
        for (int i = 0; i <= t2-1; i++) begin
          gamma_next[i] = gf_mult_a_by_b(sigma, gamma[i+1]) ^ gf_mult_a_by_b(l_poly[r], tetta[i]);
        end
        gamma = gamma_next;
      end
      tetta = gamma[0 : t2-1];
...
//    tetta[0 : t2-1] = msyndrome[0 : t2-1];
      kv      = 0;
      gvector_logout  ("ribma init g poly"  ,gamma [0 : t2]);
      gvector_logout  ("ribma init t poly"  ,tetta [0 : t2]);
      //
...
//      vector_logout   ("ribma b poly ", b_poly[0 : t2]);
//      gvector_logout  ("ribma g poly"  ,gamma [0 : t2]);
...
/*
      ...
      vector_logout ("ribma norm l poly ", l_poly_next[0 : t2]);
*/
      //
