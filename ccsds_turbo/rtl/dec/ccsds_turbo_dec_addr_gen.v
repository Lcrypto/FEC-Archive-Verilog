/*
  ...
  ptab_dat_t ccsds_turbo_dec_addr_gen__iNm1        ;
  ...
  (
    ...
    .iNm1     ( ccsds_turbo_dec_addr_gen__iNm1     ) ,
  ...
  assign ccsds_turbo_dec_addr_gen__iNm1     = '0 ;
...
// Workfile      : ccsds_turbo_dec_addr_gen.v
...
(
  ...
  input  logic      ienable     ;
  //
  input  ptab_dat_t iNm1        ; // block size in duobit
  input  ptab_dat_t iK2         ; // block size in duobit
  input  ptab_dat_t iP      [4] ; // for DVB    {P0, (N/2 + P1 + 1) % N,              P2 + 1,  (N/2 + P3 + 1) % N}

  input  ptab_dat_t iPcomp  [4] ;
  //
  output ptab_dat_t osaddr      ; // systematic pair address
  output ptab_dat_t opaddr      ; // parity pair address
  ...
  assign osaddr  = pmode ? {paddr[cW-1 : 1], !addr[0]} : addr;
  assign opaddr  = addr;
