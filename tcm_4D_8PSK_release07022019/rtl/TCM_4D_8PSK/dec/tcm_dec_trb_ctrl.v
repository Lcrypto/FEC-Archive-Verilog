/*
  ...
  trel_statem_t   tcm_dec_trb_ctrl__istatem      [64] ;
  trel_decision_t tcm_dec_trb_ctrl__idecision    [64] ;
  ...
  (
    ...
    .otrb_fdecision  ( tcm_dec_trb_ctrl__otrb_fdecision  )
    ...
    .otrb_decision   ( tcm_dec_trb_ctrl__otrb_decision   )
  );
  ...
  assign tcm_dec_trb_ctrl__iwtag           = '0 ;
...
(
  ...
  input  logic           ival                 ;
  input  logic           ieop                 ;
  input  trel_statem_t   istatem         [64] ;
  input  trel_decision_t idecision       [64] ;
  ...
  output logic           owsop                ;
  output logic           oweop                ;
  output trel_decision_t owdecision      [64] ;
  ...
  always_ff @(posedge iclk or posedge ireset) begin
    if (ireset) begin
      ...
      if (!otrb_start & (trb_start_line[cDELAY-1] | trb_fstart_line[cDELAY-1]))
        otrb_start <= 1'b1;
      else if (otrb_start & itrb_engine_rdy & (trb_start_line[cDELAY-1] | trb_fstart_line[cDELAY-1]))
        otrb_start <= 1'b1;
      else if (otrb_start & itrb_engine_rdy)
        otrb_start <= 1'b0;
      //
      if (!otrb_flush & trb_flush_line[cDELAY-1])
        otrb_flush <= 1'b1;
      else if (otrb_flush & !otrb_start & itrb_engine_rdy)
        otrb_flush <= 1'b0;
    end
  end
